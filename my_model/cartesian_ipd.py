...
def torque_cartesian(robot, sim, k, d, eef_name, desired_pos, desired_ori):
    ...
    coef = np.dot(M, J_inv)
    xd_error = np.concatenate([-x_pos_vel, -x_ori_vel])     # - dot_x
    sum = np.multiply(d, xd_error)                          # D*(- dot_x)
    pos_error = desired_pos - x_pos
    ori_error = orientation_error(desired_ori, x_ori)
    x_error = np.concatenate([pos_error, ori_error])        # x_d - x

    sum += np.multiply(k, x_error)                          # K*(x_d - x)
    sum -= np.dot(np.dot(Md, Jd), qd)                       # -M_d*J_dot*q_dot
    tau += np.dot(coef, sum)
