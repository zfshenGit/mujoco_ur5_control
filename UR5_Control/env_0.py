...
def get_jacobian():
    jacobian_pos = sim.data.get_body_jacp('wrist_3_link').reshape(3, 6)
    jacobian_ori = sim.data.get_body_jacr('wrist_3_link').reshape(3, 6)
...
def get_coriolis_gravity():
    print('coriolis_gravity len:', len(sim.data.qfrc_bias))
    print('ctrl len:', len(sim.data.qvel))
    return sim.data.qfrc_bias[:6]

def joint_velocity_control(step_nums, joint_target_vel):
    for i in range(step_nums):
        coriolis_gravity = get_coriolis_gravity()
        sim.data.ctrl[:6] = coriolis_gravity
        sim.data.qvel[:6] = joint_target_vel
...
if __name__ == '__main__':
    model = mp.load_model_from_path('./urdf/ur5_robotiq85.xml')
    sim = mp.MjSim(model)
    ...
    pos, ori = forward_kinematics('wrist_3_link')    # end effector is ee_link
    print('pos:', pos)
    print('ori:', ori)
    ...
    # target_vel = np.array([0.0, 0.0, 0.02, 0.0, 0.0, 0.0])
    # cartesian_velocity_control(5000, target_vel)
