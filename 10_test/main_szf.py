...
if __name__ == '__main__':
    ...
    win = pg.GraphicsLayoutWidget(show=True, title="End Effect Position")
    win.resize(8000, 200)
    ...
    time.timeout.connect(update_x)
    time.timeout.connect(update_y)
    time.timeout.connect(update_z)
    time.timeout.connect(update_xy)
    time.start(1)

    while 1:
        desired_pos[0] = pos[0] + 0.1*math.cos(steps/180*np.pi)
        desired_pos[1] = pos[1] + 0.1*math.cos(steps/180*np.pi)
